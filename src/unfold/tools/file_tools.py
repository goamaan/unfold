...
def get_file_tools(binary_path: Path) -> list[tuple[dict, Callable]]:
    ...
    def binary_info_handler(**kwargs) -> str:
        ...
        # Try otool (macOS)
        try:
            r = subprocess.run(["otool", "-h", str(binary_path)], capture_output=True, text=True, timeout=10)
            parts.append("=== Mach-O Header ===\n" + r.stdout.strip())
            r = subprocess.run(["otool", "-L", str(binary_path)], capture_output=True, text=True, timeout=10)
            parts.append("\n=== Shared Libraries ===\n" + r.stdout.strip())
            return "\n".join(parts)
        ...
        # Try readelf (Linux)
        try:
            r = subprocess.run(["readelf", "-h", str(binary_path)], capture_output=True, text=True, timeout=10)
            return "=== ELF Header ===\n" + r.stdout.strip()
        except FileNotFoundError:
