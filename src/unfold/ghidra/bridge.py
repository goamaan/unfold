...
from unfold.errors import GhidraError
...
_DEFAULT_JAVA_HOME = "/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home"
...
def _ensure_ghidra_started(
    ...
    try:
        ...
        raise GhidraError("PyGhidra is not installed. Install it with: pip install pyghidra") from e

    if not pyghidra.started():
        from unfold.utils import find_ghidra_install

        install_dir = find_ghidra_install(ghidra_install_dir)
        if install_dir is None:
            raise GhidraError(
                "Ghidra installation not found. Set GHIDRA_INSTALL_DIR or "
                "install Ghidra in a standard location (/opt/homebrew/Cellar/ghidra/, "
                "/usr/local/ghidra, /opt/ghidra, ~/ghidra)."
            )

        resolved_java_home = java_home or os.environ.get("JAVA_HOME") or _DEFAULT_JAVA_HOME
        ...
        try:
            ...
            if "OutOfMemoryError" in str(e) or "java.lang.OutOfMemoryError" in str(e):
                raise GhidraError(
                    "JVM ran out of memory. Try increasing heap size:\n"
                    "  export _JAVA_OPTIONS='-Xmx4g'\n"
                    "Note: the JVM cannot be restarted in the same process."
                ) from e
            raise GhidraError(f"Failed to start PyGhidra: {e}") from e
...
def _shutdown_jvm():
    """Best-effort JVM shutdown on process exit."""
    try:
        import jpype
...
class GhidraBridge:
    ...
    def _open_project(self, binary_path: Path):
        ...
        try:
            ...
            raise GhidraError(f"Failed to open Ghidra project at {project_location}: {e}") from e
    ...
    def _import_binary(self, project, binary_path: Path):
        ...
        try:
            ...
            raise GhidraError(f"Ghidra import failed for '{binary_path.name}': {e}") from e

        if load_results is None:
            raise GhidraError(f"Ghidra failed to import '{binary_path}' — unsupported format?")

        primary = load_results.getPrimaryDomainObject()
        if primary is None:
            raise GhidraError(f"No primary program from import of '{binary_path}'")
    ...
    def _analyze_program(self, program):
        ...
        if GhidraProgramUtilities.shouldAskToAnalyze(program):
            ...
            try:
                ...
                raise GhidraError(f"Analysis failed for {program.getName()}: {e}") from e

    @contextmanager
    def _open_program(self, binary_path: Path):
        """Context manager that opens a project+program and ensures cleanup."""
    ...
    def analyze(self, binary_path: Path) -> dict:
        ...
        with self._open_program(binary_path) as program:
            result = {
    ...
    def rename_function(self, binary_path: Path, target: str, new_name: str) -> dict:
        """Rename a function."""
        binary_path = Path(binary_path).resolve()
        project = self._open_project(binary_path)
        try:
    ...
    def _resolve_address(self, program, target: str):
        ...
        try:
            if target.startswith("0x"):
                ...
                addr_val = int(target)
            return program.getAddressFactory().getDefaultAddressSpace().getAddress(addr_val)
